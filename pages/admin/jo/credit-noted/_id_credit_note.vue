    ...
    >
                ...
                <tr>
                  ...
                  <td class="text-center">
                    ...
                    >
                      <template v-slot:activator="{ on, attrs }">
                        <v-icon
                          @click="destroy(req.id_credit_cost)"
                          v-bind="attrs"
                          v-on="on"
                        >
                          mdi-delete
      ...
      >
        <v-btn
          ...
          height='55px'
          class="mr-10"
          :to='`/admin/jo/detail/${this.$route.query.id}`'
        >
          Back
        </v-btn>
        <v-btn
...
export default {
  ...
  data () {
    return {
      ...
      ppn: true,
      cost: {
        id_credit_note: this.$route.params.id_credit_note,
        item_cost: '',
        unit: '',
        quantity: '',
        price: '',
  ...
  methods: {
    ...
    async updatePayreq () {
      try {
        const response = await this.$axios.put('jo-creditnote/' + this.$route.params.id_credit_note, this.payReq)
        if (response.status === 200) {
          console.log(response, 'ini hasil ngeupdate credit note')
          this.$toast.global.app_success('successfully updated this credit note')
          this.getPayreq()
        }
      } catch (error) {
        ...
        this.$toast.global.app_error(`Failed to update credit note ${message}`)
      }
    },
    async updateCost () {
      ...
      try {
        const response = await this.$axios.put('creditnote-cost/' + id, this.cost)
        if (response.status === 200) {
        ...
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to update credit note cost, ${message}`)
      }
    ...
    async deleteCost () {
      const id = this.id_cost
      try {
        const response = await this.$axios.delete('creditnote-cost/' + id)
        if (response.status === 200) {
          const { data } = response.data
        ...
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to delete credit note cost, ${message}`)
      }
    ...
    async edit (id) {
      ...
      this.id_cost = id
      try {
        const response = await this.$axios.get('creditnote-cost/' + id)
        if (response.status === 200) {
          const { data } = response.data
          this.cost = data.creditNoteCost
        ...
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to load credit note cost, ${message}`)
      }
    },
    async getPayreq () {
      this.totAmUsd = 0 
      ...
      try {
        ...
        const response = await this.$axios.get('jo-creditnote/' + this.$route.params.id_credit_note)
        if (response.status === 200) {
          this.$toast.clear()
          const { data } = response.data
          data.creditNoteCost.forEach((el) => {
          ...
          this.payReq = data
          this.payRequest = data.creditNoteCost
          // console.log(this.totPPNIdr, 'ini ppn idr', this.totPPNUsd, 'ini ppn usd')
          console.log(data, '<------ ini responsenya bila berhasil dari mendapatkan payreq')
        }
      } catch (error) {
        ...
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to load credit note ${message}`)
      }
    },
    async getDataCost () {
      try {
    ...
    async createPayreq (e) {
      ...
      try {
        const response = await this.$axios.post('creditnote-cost', this.cost)
        if (response.status === 200) {
          const { data } = response.data
          console.log(data, 'ini berhasil create credit note cost')
          this.getPayreq()
          this.cost = {
            id_credit_note: this.$route.params.id_credit_note,
            item_cost: '',
        ...
        console.log(error, 'ini error create payreqcost')
        const response = error.response.data
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to create credit note cost ${message}`)
      }
      this.dialog = false
    }
  },
