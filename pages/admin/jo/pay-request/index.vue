...
export default {
  ...
  methods: {
    createPayReq (e) {
      ...
      this.$axios.$post('jo-payrequest', payload)
        .then((res) => {
          this.$store.commit('setPayreqForm', payload)
          console.log(res, 'ini hasilnya bila berhasil broh untuk create pay req')
          // console.log(res.data.id_payrequest, 'ini id payreq nya cuy')
          this.$store.commit('setid_payreq', res.data.id_payrequest)
          this.$router.push({ path: '/admin/jo/pay-request/' + res.data.id_payrequest + '/?id=' + this.id })
        })
        .catch((err) => {
          console.log(err)
        })
