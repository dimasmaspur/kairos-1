                ...
                <tr>
                  ...
                  <td class="text-center">
                    ...
                    >
                      <template v-slot:activator="{ on, attrs }">
                        <v-icon
                          @click="destroy(req.id_payrequest_cost)"
                          v-bind="attrs"
                          v-on="on"
                        >
                          mdi-delete
                    ...
                    >
                      ...
                      >
                        <v-form
                          @submit.prevent="updateCost(req.id_payrequest_cost)"
                        >
                        <v-card-title>
                          <v-icon style="color:#3273dc;">mdi-file-document</v-icon><span class="headline" style="color:#3273dc;">Data Cost</span>
                        </v-card-title>
...
export default {
  ...
  data () {
    return {
      ...
      ppn: true,
      cost: {
        id_payrequest: this.$route.params.id_payrequest,
        item_cost: '',
        unit: '',
        quantity: '',
        price: '',
  ...
  methods: {
    ...
    async updatePayreq () {
      try {
        const response = await this.$axios.put('jo-payrequest/' + this.$route.params.id_payrequest, this.payReq)
        if (response.status === 200) {
          console.log(response, 'ini hasil ngeupdate pay req')
          this.$toast.global.app_success('successfully updated this pay request')
          this.getPayreq()
        }
      } catch (error) {
        ...
        this.$toast.global.app_error(`Failed to update pay request ${message}`)
      }
    },
    async updateCost () {
      ...
      try {
        const response = await this.$axios.put('payrequest-cost/' + id, this.cost)
        if (response.status === 200) {
    ...
    async deleteCost () {
      const id = this.id_cost
      try {
        const response = await this.$axios.delete('payrequest-cost/' + id)
        if (response.status === 200) {
          const { data } = response.data
          console.log(data, '<---- respons berhasil update')
          this.getPayreq()
    ...
    async edit (id) {
      ...
      this.id_cost = id
      try {
        const response = await this.$axios.get('payrequest-cost/' + id)
        if (response.status === 200) {
          const { data } = response.data
          this.cost = data.costPayRequest
    ...
    async getPayreq () {
      ...
      try {
        ...
        const response = await this.$axios.get('jo-payrequest/' + this.$route.params.id_payrequest)
        if (response.status === 200) {
          const { data } = response.data
          this.$toast.clear()
          data.payRequestCost.forEach((el) => {
          ...
          this.payReq = data
          this.payRequest = data.payRequestCost
        ...
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to load pay request ${message}`)
      }
    },
    async getDataCost () {
      try {
    ...
    async createPayreq (e) {
      ...
      try {
        const response = await this.$axios.post('payrequest-cost', this.cost)
        if (response.status === 200) {
          const { data } = response.data
          console.log(data, 'ini berhasil create payreqcost')
          this.getPayreq()
          this.cost = {
            id_payrequest: this.$route.params.id_payrequest,
            item_cost: '',
        ...
        const message =
          response && response.message && ` , ${response.message}`
        this.$toast.global.app_error(`Failed to load create pay req cost ${message}`)
      }
      this.dialog = false
    }
  },
