    ...
    >
            ...
            <template v-slot:default>
              ...
              >
                <tr>
                  ...
                  <!-- <td class="text-center" v-if="req.currency === 'USD'">{{ RE.ppn_usd }}%</td>
                  <td class="text-center" v-else>{{ RE.ppn_idr }}%</td> -->
      ...
      >
        <v-btn
          ...
          height='55px'
          class="mr-10"
          :to='`/admin/joc/data/${this.$route.query.id}`'
        >
          Back
        </v-btn>
        <v-btn
...
export default {
  ...
  methods: {
    ...
    async edit (id) {
      ...
      this.id_cost = id
      try {
        const response = await this.$axios.get('joc-payrequest-cost/' + id)
        if (response.status === 200) {
          const { data } = response.data
          this.cost = data.jocPayRequestCost
        ...
        this.$toast.global.app_error(`Failed to load pay request cost ${message}`)
      }
    },
    destroy (id) {
    ...
    async deleteCost () {
      ...
      try {
        const response = await this.$axios.delete('joc-payrequest-cost/' + id)
        if (response.status === 200) {
          const { data } = response.data
    ...
    async get () {
      ...
      try {
        ...
        const response = await this.$axios.get('joc-payrequest/' + this.$route.params.id)
          if (response.status === 200) {
            this.$toast.clear()
            const { data } = response.data
            data.jocPayRequestCost.forEach((el) => {
            ...
            this.RE = data
            this.REC = data.jocPayRequestCost
    ...
    async updateCost () {
      ...
      const id = this.id_cost
      try {
        const response = await this.$axios.put('joc-payrequest-cost/' + id, this.cost)
        if (response.status === 200) {
          const { data } = response.data
          this.cost = {}
    ...
    //   console.log('ngebikin')
      ...
      this.cost.id_payrequest = this.$route.params.id
      console.log(this.cost, 'ini bawaan post')
      try {
        const response = await this.$axios.post('joc-payrequest-cost', this.cost)
        if (response.status === 200) {
          this.get()
          this.dialog = false
    ...
    async update (e) {
      ...
      try {
        const response = await this.$axios.$put('joc-payrequest/' + this.$route.params.id , this.RE)
        if (response.status = 200) {
